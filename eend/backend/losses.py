...
def pit_loss_multispk(
    ...
    # Fix: Prevent in-place operations by using target.clone()
    target = target.clone()
    ...
    if detach_attractor_loss:
        ...
        for i in range(target.shape[0]):
            target[i, :, n_speakers[i]:] = -1 * torch.ones(
                          target.shape[1], target.shape[2]-n_speakers[i], device=target.device)
    ...
    for i, cost_mx in enumerate(cost_mxs.detach().cpu().numpy()):  # Added detach()
        ...
        # Fix: Prevent in-place operations and adjust dimensions
        target[i, :] = target[i, :].index_select(dim=1, index=torch.tensor(ref_alig, device=target.device))
    ...
    for i in range(target.shape[0]):
        loss[i, n_speakers[i]:] = torch.zeros(loss.shape[1]-n_speakers[i], device=loss.device)
...
def vad_loss(ys: torch.Tensor, ts: torch.Tensor) -> torch.Tensor: #def dummy():
